...
Перем iCaller;
Перем НомерИБ Экспорт;

////////////////////////////////////////////////////////////////////////////
// Программный интерфейсе

Процедура Инициализировать(Знач МодульУправления) Экспорт
	
	iCaller = МодульУправления;
	мНастройки = iCaller.Настройки();
	ПрочитатьНастройки();
КонецПроцедуры
...
Функция ЕстьРаботающиеСеансы(Знач Дескриптор, Знач ТихаяПроверка = Ложь, Знач ИгнорироватьКонсольКластера = Истина) Экспорт
	...
	ТекРежимВыдачиСообщений = iCaller.ВыдаватьСообщенияСборки();
	iCaller.ВыдаватьСообщенияСборки(Не ТихаяПроверка);
	...
	Попытка
		...
		ОсвободитьОбъектКластера(InfobaseDescriptor);
	Исключение
		iCaller.ВыдаватьСообщенияСборки(ТекРежимВыдачиСообщений);
		ВызватьИсключение;
	КонецПопытки;
...
Процедура ПрочитатьНастройки() Экспорт
	...
	// Параметры сервера
	ХостСервера = мНастройки[iCaller.ПреобразоватьИмяНастройки("param_server.host")];
	ПортСервера = 1541;
	...
	ПортАгента = мНастройки[iCaller.ПреобразоватьИмяНастройки("param_agent.port")];
	...
	мНастройки.Вставить("АдминистраторКластера", мНастройки[iCaller.ПреобразоватьИмяНастройки("param_cluster.admin")]);
	мНастройки.Вставить("ПарольАдминистратораКластера", мНастройки[iCaller.ПреобразоватьИмяНастройки("param_cluster.admin.password")]);
	мНастройки.Вставить("КлассCOMСоединения", мНастройки[iCaller.ПреобразоватьИмяНастройки("param_com.connector")]);
	
	// Параметры рабочей базы
	мНастройки.Вставить("ИмяБазы", мНастройки[iCaller.ПреобразоватьИмяНастройки("ib_ib."+НомерИБ)]);
	мНастройки.Вставить("АдминистраторБазы", мНастройки[iCaller.ПреобразоватьИмяНастройки("ib_user."+НомерИБ)]);
	мНастройки.Вставить("ПарольАдминистратораБазы", мНастройки[iCaller.ПреобразоватьИмяНастройки("ib_password."+НомерИБ)]);
	
	// Прочие настройки
	мНастройки.Вставить("СообщениеБлокировки", мНастройки[iCaller.ПреобразоватьИмяНастройки("param_lock.message")]);
	мНастройки.Вставить("ТаймаутБлокировки", 1000);
